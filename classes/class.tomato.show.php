...
*/
    ...
    /*
    private function pull_tomatos_this_week()
    {
...
*/
    ...
    /*
    private function get_keywords_for_tom_id($tom_id)
    {
...
*/
    /*
    private function show_keywords_by_link_id($dbase_resource)
    {
        $return_string = "";
...
*/
    ...
    /*
    private function show_tomato_by_tomid($tom_id = 1)
    {
...
*/
    ...
    /*
    private function query_table_for_tomdate_today()
    {
...
/*
    private function return_category_name_from_catid($catid)
    {
...
/*
    private function return_single_tomato($tomid)
    {
...
*/
    private function return_category_name_by_catid($catid)
    {
        $sth = $this->conn->prepare("SELECT * FROM `tomato220`.`category` WHERE `category`.`id` = :CATID ORDER BY `id` DESC");
        $sth->bindParam(':CATID', $catid);
        $sth->execute();
    ...
    /*
    private function return_keywords_on_tomid($tomid)
    {
    ...
    */
    ...
    private function get_toms_by_DOW_and_weekno($DOW, $weekno)
    {
        ...
        print('<tr><th>Desc.</th><th>Cat.</th><th>Toms</th></tr>');
        for ($i = 0; $i < $size; $i++) {
    ...
    public function today_values()
    {
        ...
        print('<div class="days today_day">');
         print('<table class="table">');
         print('<tr><th>Description</th><th>Cat.</th><th>Toms</th></tr>');
        for ($i = 0; $i < $size; $i++) {
            $title = $resource[$i]['title'];
            $category = $resource[$i]['category'];
            $count = $resource[$i]['count'];
            $timestamp = $resource[$i]['timestamp'];
            // make timestamp human readable
            $dateTime = date('Y-m-d H:i:s', $timestamp);
            $id = $resource[$i]['id'];
            print('<tr>');
            print('<td><a href="home.php?page=tomatoedit&tomid=' . $id . '">' . $title . '</a> </td>');
            print('<td class="category_td"><span>' . $this->return_category_name_by_catid($category) . '</span></td>');
            print('<td class="tomcount_td"><span>' . $resource[$i]['count'] . '</span></td>');
            print('</tr>');
        }
       ...
       // $toms_in_hours = (.5) * ($totaltoms);
        print('</table>');
        print('<h5 class="total_hours_h5">Total Toms &nbsp;&nbsp;<span class="badge badge-secondary">'.$totaltoms.'</span></h5>');
       // print('<p align="right">Hours ' . $toms_in_hours . '<span></p>');
        print('</div>');
    }
}
